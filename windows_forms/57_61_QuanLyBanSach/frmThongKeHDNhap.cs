...
namespace _57_61_QuanLyBanSach
{
    public partial class frmThongKeHDNhap : Form
    {
        ...
        private void txtNgayThang_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if(f == 0)
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        double doanhThu = 0;
                        string a = "select * from HoaDonNhap where Day(NgayLapHD) = '" + txtNgay.Text + "' and Month(NgayLapHD) = '" + txtThang.Text + "' and Year(NgayLapHD) = '" + txtNam.Text + "'";
            ...
            if(f == 1)
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        double doanhThu = 0;
                        string a = "select * from HoaDonNhap where Month(NgayLapHD) = '" + txtThang.Text + "' and Year(NgayLapHD) = '" + txtNam.Text + "'";
                        danhsach_datagridview(dgvDanhSach, a);
                        for (int i = 0; i < dgvDanhSach.Rows.Count - 1; i++)
                        {
                            doanhThu += double.Parse(dgvDanhSach.Rows[i].Cells[2].Value.ToString());
            ...
            if (f == 2)
            {
                ...
                {
                    double doanhThu = 0;
                    string a = "select * from HoaDonNhap where Year(NgayLapHD) = '" + txtNam.Text + "'";
                    danhsach_datagridview(dgvDanhSach, a);
                    for (int i = 0; i < dgvDanhSach.Rows.Count - 1; i++)
                    {
                        doanhThu += double.Parse(dgvDanhSach.Rows[i].Cells[2].Value.ToString());
